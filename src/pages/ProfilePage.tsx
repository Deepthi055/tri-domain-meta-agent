...
import { useAuth } from '@/contexts/AuthContext'
import { useLanguage } from '@/contexts/LanguageContext'
import { authService, api, getErrorMessage, memoryService, profileService } from '@/services'
import { useProfile, queryKeys, invalidateProfileDependentQueries } from '@/hooks'
import { PageHeader } from '@/components/layout/PageHeader'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
...
const profileSchema = z.object({
  ...
  career: z.object({
    ...
    preferred_roles: z.string().optional(),
    resume: z.string().optional(),
    // FIX: resume_text was missing entirely from the schema, the payload
    // builder, and the form-value builder - even a perfect extraction in
    // handleResumeUpload had nowhere to go. This is the field
    // resume_optimizer() actually needs; `resume` stays as just the
    // filename/display reference.
...
function buildProfilePayload(data: ProfileForm): FullProfile {
  return {
    ...
    career: data.career ? {
      ...
      preferred_roles: data.career.preferred_roles,
      resume: data.career.resume,
      resume_text: data.career.resume_text,
    } : undefined,
    health: data.health ? {
      medical_conditions: data.health.medical_conditions,
      lifestyle: data.health.lifestyle,
...
function buildFormValues(profile: FullProfile | undefined, userName: string): ProfileForm {
  ...
  return {
    ...
    career: profile.career ? {
      ...
      preferred_roles: profile.career.preferred_roles ?? undefined,
      resume: profile.career.resume ?? undefined,
      resume_text: profile.career.resume_text ?? undefined,
    } : {},
    health: profile.health ? {
      medical_conditions: profile.health.medical_conditions ?? undefined,
      lifestyle: profile.health.lifestyle ?? undefined,
    ...
    try {
      ...
      if (isTextFile) {
        ...
        try {
          ...
          console.warn('Could not read file as text:', readErr)
          throw new Error('Could not read this file as text.')
        }
      } else {
        // PDF/DOCX/etc: the browser can't parse these formats, so the raw
        // file is sent to the backend, which extracts text server-side
        // with pdfplumber/python-docx and returns it. This was the gap
        // before - binary files previously only ever stored a filename
        // reference, never real content.
        //
        // Uses the shared `api` axios instance (from '@/services/api')
        // instead of raw fetch, so the request interceptor auto-attaches
        // Authorization: Bearer <token> from sessionStorage, and the
        // response interceptor's 401 handling (clear session, redirect
        // to login) applies automatically if the token's expired.
        const formData = new FormData()
        formData.append('file', file)

        const response = await api.post('/profile/resume', formData, {
          // The `api` instance sets a default Content-Type of
          // application/json - that must be cleared for this request so
          // axios/the browser can set the correct
          // multipart/form-data; boundary=... header instead. Setting it
          // to undefined here removes the default rather than sending a
      ...
      // `resume` = short display reference (filename), consistently for
      // both text and binary uploads now. `resume_text` = full content,
      // what resume_optimizer() actually analyzes.
      setValue('career.resume', file.name)
      setValue('career.resume_text', extractedText)

      try {
        await memoryService.create({
          memory_text: `Career resume uploaded: ${file.name}`,
          category: 'career',
          importance_score: 0.95,
        })
        setResumeUploadMessage(`Resume uploaded as ${file.name}`)
        toast.success('Resume uploaded')
      } catch (memoryErr) {
        // Memory note is a nice-to-have; resume_text is already in form
        // state and will still be saved when the profile form submits.
        console.error('Memory save error:', memoryErr)
        setResumeUploadMessage(`Resume uploaded as ${file.name} (memory note failed, but it will still save with your profile)`)
        toast.warning('Resume uploaded, but memory note failed. It will still save with your profile.')
      }
    } catch (err) {
      console.error('Resume upload error:', err)
      setResumeUploadMessage('Unable to process resume file.')
            ...
            <Card>
              ...
              <CardContent className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <input type="hidden" {...register('career.resume')} />
                  <input type="hidden" {...register('career.resume_text')} />
                </div>
              </CardContent>
            </Card>
          </TabsContent>
